...
def _create_topics_from_bulk(user_id, subject_id, topics_bulk):
    """Create many topics from a textarea where each line is Title|Minutes."""
    created_topics = 0
    for raw_line in topics_bulk.splitlines():
        line = raw_line.strip()
        if not line:
            continue

        if '|' in line:
            title, minutes = [part.strip() for part in line.split('|', 1)]
        else:
            title, minutes = line, '60'
        ...
        current_app.mongo.db.topics.insert_one({
...
def add_subject():
    ...
    subject_id = current_app.mongo.db.subjects.insert_one({
        'user_id': ObjectId(user_id),
        'name': name,
        'exam_date': exam_date,
    ...
    created_topics = _create_topics_from_bulk(user_id, subject_id, topics_bulk) if topics_bulk else 0

    if created_topics:
        flash(f'Subject "{name}" added with {created_topics} topics.', 'success')
    else:
...
def edit_subject(subject_id):
    ...
    current_app.mongo.db.subjects.update_one(
        {'_id': ObjectId(subject_id)},
        {'$set': {'name': name, 'exam_date': exam_date, 'difficulty': difficulty, 'color': color}}
    )
...
def delete_subject(subject_id):
    ...
    current_app.mongo.db.topics.delete_many({'user_id': ObjectId(user_id), 'subject_id': ObjectId(subject_id)})
    current_app.mongo.db.sessions.delete_many({'user_id': ObjectId(user_id), 'subject_id': ObjectId(subject_id)})
...
def manage_topics(subject_id):
    ...
    return render_template('subjects/topics.html', subject=subject, topics=topics, stats=stats)
...
def add_topic(subject_id):
    ...
    if topics_bulk:
        created_topics = _create_topics_from_bulk(user_id, ObjectId(subject_id), topics_bulk)
        flash(f'Added {created_topics} topics to {subject["name"]}.', 'success')
...
def toggle_topic(topic_id):
    ...
    current_app.mongo.db.topics.update_one({'_id': ObjectId(topic_id)}, {'$set': {'status': new_status}})
...
def delete_topic(topic_id):
    ...
    current_app.mongo.db.sessions.delete_many({'user_id': ObjectId(user_id), 'topic_id': ObjectId(topic_id)})
